# Plantilla de Experimento

> Estado: Instrumento metodológico
>
> Esta plantilla no representa un experimento ejecutado.
> Representa el instrumento que ProgramEze utiliza para diseñar, registrar y analizar experimentos.
>
> **Objetivo del instrumento**
>
> Esta plantilla no busca producir mejores observaciones.
> Busca producir observaciones más confiables.

---

# Guía de uso

Esta plantilla se completa en tres momentos distintos.

## Antes de abrir el material

Completar las secciones 1 a 6.

## Durante la observación

Cada vez que aparezca un ⚑, duplicar el bloque de Observación y registrarlo inmediatamente.

No intentar explicar el fenómeno.

Solo observarlo.

## Después de terminar la observación

Completar las secciones 8 a 11.

Las explicaciones y aprendizajes surgirán posteriormente durante el Consejo.

---

# 1. Identificación

## Nombre

Experimento Nº**\_ — ********\_\_\_\_**********

## Fecha

---

## Laboratorio

---

## Dominio observado

---

## Observador principal

---

---

# 2. Pregunta de investigación

¿Qué queremos observar?

> ---

---

# 3. Hipótesis provisional

¿Qué creemos que podría ocurrir?

> ---

La hipótesis existe para ser puesta en tensión, no para ser defendida.

---

# 4. Prerregistro

Antes de abrir el material registrar únicamente aquello que protege la falsabilidad del experimento.

## ¿Qué resultado me sorprendería?

---

## Condición que podría debilitar mi hipótesis

---

---

# 5. Contexto experimental

## Material utilizado

Presentación:

Prompt:

Fuente:

Versión:

Fecha:

## Contexto del observador

Horario:

Nivel de cansancio:

Estado emocional:

Interrupciones:

Observaciones relevantes:

---

# 6. Control de sesgos

Antes de comenzar marcar los riesgos presentes.

- [ ] Ya conozco demasiado este tipo de material.
- [ ] Espero encontrar errores.
- [ ] El material pudo estar influenciado por experimentos anteriores.
- [ ] Estoy cansado o apurado.
- [ ] Estoy emocionalmente involucrado.
- [ ] Otro:

---

# 7. Observaciones

Duplicar este bloque una vez por cada ⚑.

---

## ⚑ Observación Nº\_\_\_

### ¿Qué ocurrió?

Describir únicamente el hecho observable.

---

### ¿En qué instante apareció el "Mmm..."?

---

---

### ¿Desde qué cabeza apareció?

- [ ] Docente
- [ ] Alumno
- [ ] IA
- [ ] Relación entre dos cabezas
- [ ] No estoy seguro

---

### ¿Qué experiencia futura del alumno estoy intentando proteger?

---

---

### ¿Qué imaginé que iba a pasar?

---

---

### ¿Qué decisión tomé?

- [ ] Aceptar
- [ ] Reescribir
- [ ] Dividir
- [ ] Reordenar
- [ ] Eliminar
- [ ] Dejar en observación
- [ ] Otra:

---

### Nivel de seguridad

- [ ] Duda leve
- [ ] Intuición débil
- [ ] Intuición fuerte
- [ ] Certeza alta
- [ ] Certeza absoluta

---

### ¿Podría explicar esta decisión a otro docente?

- [ ] Sí
- [ ] Parcialmente
- [ ] No
- [ ] Todavía no

---

# 8. Explicaciones rivales

Esta sección analiza el experimento completo.

Si alguna observación requiere una explicación rival específica, registrarla dentro de la propia observación.

¿Qué explicaciones alternativas podrían existir para los fenómenos observados?

- [ ] El cansancio pudo influir.
- [ ] Esperaba encontrar errores.
- [ ] El problema puede pertenecer al layout y no al contenido.
- [ ] El material simplemente no coincide con mi estilo docente.
- [ ] Parte de las observaciones podrían ser ruido del instrumento.
- [ ] Otra:

Observaciones:

---

---

# 9. Cierre del experimento

## Hoy observé que...

---

## Hoy me sorprendió que...

---

## Algo que todavía no sé explicar...

---

## Algo que merece llevarse a la Fogata...

---

---

# 10. Pregunta para la Fogata

¿Qué necesita discutir el Consejo antes de convertir esta experiencia en un aprendizaje?

---

---

# 11. Estado final

- [ ] Experimento abierto
- [ ] Observación terminada
- [ ] Enviado al Consejo
- [ ] Aprendizaje generado
- [ ] Cerrado
